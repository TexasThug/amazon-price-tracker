...
def cache_key(asins, marketplace):
    return f"{marketplace}__{'_'.join(sorted(asins))}"

def get_cached_df(asins, marketplace):
    cache = load_cache()
    key = cache_key(asins, marketplace)
    if key not in cache:
        return None
    entry = cache[key]
...
def set_cached_df(asins, marketplace, df):
    cache = load_cache()
    key = cache_key(asins, marketplace)
    cache[key] = {
...
if not fetch_btn and not force_refresh:
    st.info("👈 Enter one or more ASINs in the sidebar and click **Fetch** to get started.")
    st.stop()
...
cached = None if force_refresh else get_cached_df(asins, marketplace)
...
if cached is not None:
    ...
    with st.spinner(f"Fetching data for {len(asins)} product(s) on {marketplace}…"):
        try:
            ...
            set_cached_df(asins, marketplace, df)
...
for i, row in summary_df.iterrows():
    ...
    with col:
        current = row["Current (£/€)"]
        change  = row["30d change %"]
        # Positive change = price went up = bad for buyer = red
        delta_color = "inverse" if change > 0 else "normal"
        st.metric(
            label=row["Product"],
            value=f"£/€ {current:.2f}",
            delta=f"{change:+.1f}% vs 30d avg",
            delta_color=delta_color,
        )
        alert = row["Alert"]
        color = "🟢" if "DROP" in alert else ("🔴" if "UP" in alert else "⚪")
        st.caption(f"{color} {alert}")
